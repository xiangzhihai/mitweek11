...
class RNN:
    ...
    def bptt(self, xs, dLtdz2, states):
        ...
        for t in range(k-1, -1, -1):
            ...
            dLtdst = np.transpose(self.Wo)@dLtdz2t
            # raise Exception("bptt implementation incomplete")   # comment this out
            # derivatives of loss from t forward
            dFtm1dst = dLtdst + dFtdst
            dFtm1dz1t = dFtm1dst * self.df1(self.Wss@stm1 + self.Wsx@xt + self.Wss0)
            dFtm1dstm1 = np.transpose(self.Wss)@dFtm1dz1t
            # gradients wrt weights
            dLtdWo = dLtdz2t@np.transpose(st)
            dLtdWo0 = dLtdz2t
            dFtm1dWss = dFtm1dz1t@np.transpose(stm1)
            dFtm1dWss0 = dFtm1dz1t
            dFtm1dWsx = dFtm1dz1t@np.transpose(xt)
...
#delay_num_test(delay = 3, seq_length = 10, num_epochs = 100000, step_size =.01)
